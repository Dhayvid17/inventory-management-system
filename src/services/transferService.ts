  ...
  try {
    ...
    const notifyStaff = async (
      ...
      for (const staff of warehouseStaff) {
        await transferRequestNotification(staff, message, "Transfer Request");
      }
    };

    await notifyStaff(
      fromWarehouseId,
      `You have a new transfer request from warehouse ${fromWarehouseId}.`,
      "Tranfer Request"
    );
    await notifyStaff(
      toWarehouseId,
      `You have sent a transfer request to warehouse ${toWarehouseId}.`,
      "Tranfer Request"
    );
...
const completeTransferRequest = async (
  ...
  try {
    ...
    const notifyStaff = async (
      ...
      for (const staff of warehouseStaff) {
        await transferRequestNotification(staff, message, type);
      }
    };
