...
const StaffAssignmentForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    //Check if staff is in any warehouse
    const isStaffInAnyWarehouse = warehouses.some((warehouse) =>
      warehouse.managedBy?.some(
        (staff) => staff._id.toString() === staffUsername
      )
    );
    if (isStaffInAnyWarehouse) {
      setError("Staff already exists in a warehouse");
      return;
    }
  ...
  return (
    ...
    >
      ...
      <button
        ...
        disabled={loading}
      >
        {loading ? "Adding..." : "Add Staff To Warehouse"}
      </button>
    </form>
  );
};
